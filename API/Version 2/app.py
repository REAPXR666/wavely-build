...
# Persistent secret key — survives restarts so sessions aren't invalidated
...
# Threading locks for DB files to prevent write collisions and corruption
auth_lock = threading.RLock()
analytics_lock = threading.RLock()
beat_battles_lock = threading.RLock()
servers_lock = threading.RLock()
dms_lock = threading.RLock()
...
def api_community_poll():
    ...
    if active_call:
        ...
        save_dms(dms_data)
    return jsonify({"announcements": announcements, "incoming_call": incoming_call, "active_call": active_call})
...
if __name__ == '__main__':
    ...
    print("Wavely V2 Flask API Server running on http://127.0.0.1:5000")
